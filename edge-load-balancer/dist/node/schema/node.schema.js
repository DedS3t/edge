"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.NodeSchema = exports.Node = void 0;
const mongoose_1 = require("@nestjs/mongoose");
const mongoose = require("mongoose");
const user_schema_1 = require("../../user/schema/user.schema");
let Node = class Node {
};
__decorate([
    mongoose_1.Prop({ type: mongoose.Schema.Types.ObjectId, ref: 'User' }),
    __metadata("design:type", user_schema_1.User)
], Node.prototype, "owner", void 0);
__decorate([
    mongoose_1.Prop(),
    __metadata("design:type", String)
], Node.prototype, "host", void 0);
__decorate([
    mongoose_1.Prop(),
    __metadata("design:type", Number)
], Node.prototype, "port", void 0);
__decorate([
    mongoose_1.Prop({ enum: ['unknown', 'offline', 'ready', 'busy'] }),
    __metadata("design:type", String)
], Node.prototype, "state", void 0);
__decorate([
    mongoose_1.Prop(),
    __metadata("design:type", Date)
], Node.prototype, "last_state_update", void 0);
__decorate([
    mongoose_1.Prop({ unique: true }),
    __metadata("design:type", String)
], Node.prototype, "host_port", void 0);
__decorate([
    mongoose_1.Prop(),
    __metadata("design:type", Number)
], Node.prototype, "mem_avail", void 0);
__decorate([
    mongoose_1.Prop(),
    __metadata("design:type", Number)
], Node.prototype, "cpu_avail", void 0);
Node = __decorate([
    mongoose_1.Schema()
], Node);
exports.Node = Node;
exports.NodeSchema = mongoose_1.SchemaFactory.createForClass(Node);
//# sourceMappingURL=node.schema.js.map